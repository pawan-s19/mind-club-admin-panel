...
export default function WorkshopLocationCard({ location, workshopId }) {
    ...
    const [form, setForm] = useState({
        ...
        locationBlog: location?.locationBlog
            ? location.locationBlog.map((v) => ({
                  ...v,
                  imageOrVideo: { ...v.imageOrVideo },
              }))
            : [],
    });
    ...
    const handleVisualChange = (index, field, value) => {
        setForm((prev) => {
            const updated = [...prev.locationBlog];
            if (field === "imageOrVideo.url") {
                updated[index].imageOrVideo.url = value;
            } else {
                updated[index][field] = value;
            }
            return { ...prev, locationBlog: updated };
        });
    };

    // Handle file input for imageOrVideo.url
    const handleVisualFileChange = (index, e) => {
        const file = e.target.files?.[0];
        if (!file) return;
        const reader = new FileReader();
        reader.onloadend = () => {
            handleVisualChange(index, "imageOrVideo.url", reader.result);
        };
        reader.readAsDataURL(file);
    };
    ...
    const handleRemoveVisual = (index) => {
        setForm((prev) => ({
            ...prev,
            locationBlog: prev.locationBlog.filter((_, i) => i !== index),
        }));
    };
            ...
            <h4 className="mb-2 text-lg font-semibold text-center text-gray-800 dark:text-white/90 xl:text-left">
                ...
                {/* Responsive grid for workshopVisual cards */}
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-6 w-full">
                                {location?.locationBlog && location.locationBlog.map((value, index) => (
                ...
                <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
                    <div className="px-2 pr-14">
                        ...
                        <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                            Update your details to keep your location up-to-date.
                        </p>
                    </div>
                        ...
                        <div className="custom-scrollbar h-[450px] overflow-y-auto px-2 pb-3">
                            <div>
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-6 w-full">
                                        {form.locationBlog.map((value, index) => (
                                            <div className="bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 flex flex-col" key={index}>
                                                <div className="w-full h-40 flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded-t-lg overflow-hidden">
                                                    {value.imageOrVideo.url && (
