...
interface WorkshopCreatorProps {
    creator: any; // Replace 'any' with your actual creator type if available
    workshopId: string;
}

export default function WorkshopCreator({ creator, workshopId }: WorkshopCreatorProps) {
    ...
    const [form, setForm] = useState({
        ...
        imageOrVideo: creator?.imageOrVideo || [] // expects [{ url, _id? }]
    });
    ...
    // Change a field in a imageOrVideo item
    const handleVisualChange = (index: number, field: string, value: any) => {
        setForm(prev => {
            const updated = [...(prev.imageOrVideo || [])];
            if (field === "url") {
                if (updated[index]) {
            ...
            return { ...prev, imageOrVideo: updated };
        });
    };
    ...
    const handleVisualFileChange = (index: number, e: ChangeEvent<HTMLInputElement>) => {
        ...
        const reader = new FileReader();
        reader.onloadend = () => {
            handleVisualChange(index, "url", reader.result as string);
        };
        reader.readAsDataURL(file);
    };

    // Add a new imageOrVideo item
    ...
    // Remove a imageOrVideo item
    const handleRemoveVisual = (index: number) => {
        setForm(prev => ({
            ...prev,
            imageOrVideo: (prev.imageOrVideo || []).filter((_: any, i: number) => i !== index)
        }));
    };
    ...
    return (
        <>
            ...
            <h4 className="mb-2 text-lg font-semibold text-center text-gray-800 dark:text-white/90 xl:text-left">
                ...
                {/* Responsive grid for imageOrVideo cards */}
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-6 w-full">
                    {creator?.imageOrVideo && creator.imageOrVideo.map((value: any, index: number) => (
                        <div className="bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 flex flex-col" key={index}>
                            <div className="w-full h-40 flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded-t-lg overflow-hidden">
                                {/* {value.imageOrVideo && value.imageOrVideo.url && ( */}
                                    <img className="object-cover h-full w-full" src={value.url} alt={value?.name || ''} />
                                {/* )} */}
                            </div>
                        </div>
                    ))}
                ...
                <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
                    <div className="px-2 pr-14">
                        ...
                        <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                            Update your details to keep your creator up-to-date.
                        </p>
                    </div>
                        ...
                        <div className="custom-scrollbar h-[450px] overflow-y-auto px-2 pb-3">
                            <div>
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    {/* Editable grid for imageOrVideo */}
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-6 w-full">
                                        {form.imageOrVideo.map((value: any, index: number) => (
                                            <div className="bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 flex flex-col" key={index}>
                                                <div className="w-full h-40 flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded-t-lg overflow-hidden">
                                                        <img src={value.url} alt="" className="object-cover h-full w-full" />
                                                </div>
